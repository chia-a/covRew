...
def cardinality_estimation_old(sample, table_size, q, sens_attr, sens_values):
...
def cardinality_estimation(sample, table_size, q, k):
    ...
    print(est_card_g, tuple(sens_values_count))
    return (est_card_g, tuple(sens_values_count))
...
def axes_search(k, sample, card_tot, relax_attributes, boolean_op):
    ...
    for i, v in enumerate(relax_attributes):
        ...
        while j < min([len(x['bins']) for x in relax_attributes]):
            ...
            res = (cardinality_estimation(sample, card_tot, query, [*k]), low)
            count_stime = count_stime + 1
            ## res è una tupla con stime e indice cella ( (stima_tot, (stime_as)), idx_cella )
            # --> in res[0] ci sono le stime - res[0][0]:stima totale, res[0][1]: stime as
            # --> in res[1] indice cella
            if all([x >= int(k[i]['num']) for i, x in enumerate(res[0][1])]):
            # if res[0][1][0] >= k['card']:
...
def minimum_index_2(k, locks, nosols, sample, card_tot, relax_attributes, boolean_op, n_bin, iter):
...
###############################################################################
def cmg_query_ref(sample, relax_attributes, boolean_op, table_size, sample_size, n_bin, sens_attr_list, k, prep):#, pruning, iter):
    print('input', relax_attributes, table_size, sample_size, n_bin, sens_attr_list, k)
    orig_query = boolean_op.join([x['attr'] + ' ' + x['op'] + ' ' + x['val_orig'] for x in relax_attributes])

    # sens_attr_values = sample[sens_attr_list].unique()
    # sens_attr_counts = {}
    init_card_est = cardinality_estimation(sample, table_size, orig_query,k)
    print('La cardinalità stimata per la query originale è:', init_card_est, init_card_est[0] , init_card_est[1] , '\n')

    # qui salvo la lista delle cardinlita stimate per i vincoli dati
    print(list(init_card_est[1]))
...
#################################### PREPROCESSING #################################
    ...
    print('CONTROLLO SE IL VINCOLO è già soddisfatto ')
...
########### ALGO ##########################
    min_idx, count_q = minimum_index_2(k, locks, nosols, sample, table_size, relax_attributes, boolean_op, n_bin, iter)
...
def query_rewrite_CC(data, info_query):
    ...
    print('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA', info_query['binary_op'])
    #boolean_op_init = info_query['binary_op']
    boolean_op = ''

    if len(relax_attributes) > 1 and (' OR ' == info_query['binary_op'] or ' or ' == info_query['binary_op']) and (' AND ' == info_query['binary_op'] or ' and ' == info_query['binary_op']):
        print('Conditions must be all ANDs or all ORs!')
        exit(1)
    elif ' OR ' == info_query['binary_op']:
        boolean_op = ' | '
    elif ' AND ' == info_query['binary_op']:
        boolean_op = ' & '

######### vincoli
    sens_attr_cc = []
    for i in range(0, len(info_query['CC'])):
        for ii in range(0, len(info_query['CC'][i]['AS'])):
            sens_attr_cc += [info_query['CC'][i]['AS'][ii]]
    sens_attr_list = list(set(sens_attr_cc))
    print('sens attr list', sens_attr_list)

    sens_attr = sens_attr_list
    k = info_query['CC']
    print(sens_attr_list, k)
    ...
    if all(elem in sample.columns for elem in sens_attr_list) == False:
        metadata_cmg = {'cmg_info': relax_attributes, 'n_bin': n_bin, 'solution_idx': "Query rewriting is not possible: at least one of the sensitive attributes does not appear in the projection list", 'orig_query': boolean_op.join([x['attr'] + ' ' + x['op'] + ' ' + x['val_orig'] for x in relax_attributes])}
        rewritten_query = None
        res_data = metadata_cmg['solution_idx']
        return rewritten_query, metadata_cmg, res_data
    else:
        # rewriting
        metadata_cmg = cmg_query_ref(sample, relax_attributes, boolean_op, table_size, sample_size, n_bin, sens_attr_list, k, prep)
    ...
    for cond in k:
        condition = (' & ').join([sens_attr + ' == \'' + cond['value'][x] + '\'' for x, sens_attr in enumerate(cond['AS'])])
        card_vera_as_Q.append(len(data.query('('+ metadata_cmg['orig_query']  + ') & ' + condition)))

    metadata_cmg['card_tot_init'] = card_vera_tot_Q
    metadata_cmg['card_as_init'] = card_vera_as_Q


    if metadata_cmg['solution_idx'] == "The coverage constraint is already satisfied" or metadata_cmg['solution_idx'] == "It is not possible to rewrite the query":
        rewritten_query = None
        res_data = metadata_cmg['solution_idx']
    else:
        # 1) rewritten query
        ...
        for cond in k:
            condition = (' & ').join([sens_attr + ' == \'' + cond['value'][x] + '\'' for x, sens_attr in enumerate(cond['AS'])])
            card_vera_as_newQ.append(len(res_data.query(condition)))
        # print(card_vera_as_newQ)
        ...
        metadata_cmg['solution_idx'] = list(map(lambda xx: int(xx), metadata_cmg['solution_idx']))

    print('Query riscritta',rewritten_query)
    print('METADATI', metadata_cmg)
